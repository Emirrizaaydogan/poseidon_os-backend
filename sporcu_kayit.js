module.exports = function sporcuKayitKur(app, pool, bcrypt, girisGerekli, sadeceAntrenor) {
  ...
  app.post('/athletes/onboard', girisGerekli, sadeceAntrenor, async (req, res) => {
    ...
    try {
      ...
      res.status(e.code === '23505' ? 409 : e.status || 500).json({mesaj:
        e.code === '23505' ? 'Bu hesap zaten mevcut' : e.status ? e.message : 'Sporcu kaydedilemedi'});
